# Google Ads MCP Server — Implementation Instructions

**Owner:** Antigravity
**Date:** March 2026
**Dependency:** `@vercel/mcp-adapter` and `zod` must be installed (already done)

---

## Context

This adds a Model Context Protocol (MCP) server to the existing Next.js project. Once deployed, it allows Claude AI to query AnalytixLabs Google Ads campaigns conversationally — campaign stats, keyword performance, search terms, conversion breakdowns, and budget pacing.

The MCP server is a single new API route added to the existing Vercel project. No new infrastructure needed.

---

## Step 1 — Create the MCP Route File
...
git commit -m "feat: add Google Ads MCP server with 5 tools"
git push
```
...
After Vercel deploys, confirm the route is live:

```bash
curl https://lp-vercel.analytixlabs.co.in/api/mcp
```

You should get a non-404 response (even an error response is fine — it confirms the route exists).
...
|---|---|
| `get_campaign_stats` | "Show me all campaigns with conversions in last 30 days" |
| `get_keyword_stats` | "Which keywords have CPA above ₹800?" |
| `get_search_terms` | "What did people search before clicking our BLR ads?" |
| `get_conversion_stats` | "Which CTA section is converting the most?" |
| `get_budget_pacing` | "Are any campaigns over or under budget this week?" |

---

## Checklist

- [ ] `app/api/mcp/route.ts` created with exact code above
- [ ] Committed to correct file path
- [ ] Pushed to GitHub
- [ ] Vercel deployment confirmed
- [ ] Curl test on `/api/mcp` returns non-404
