        ...
        <section id="faq" className="py-14 bg-white">
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6">
                <div className="text-center mb-10">
                    <span className="inline-block bg-[#e8f4fd] text-[#00AEEF] text-[12px] font-bold uppercase tracking-[0.1em] px-[18px] py-[5px] rounded-full border border-[#b8ddf7]">Support</span>
                    <h2 className="text-[#09263F] font-bold text-3xl sm:text-4xl mt-4 mb-2">Frequently <span className="bg-gradient-to-r from-[#19dfaf] to-[#07b2e8] bg-clip-text text-transparent">Asked</span> Questions</h2>
                    <p className="text-[#4A6275] max-w-md mx-auto">Common Questions</p>
                </div>

                <div className="grid md:grid-cols-2 gap-4 items-start">
                    {FAQ_DATA.map((faq, i) => (
                        <div key={i} className={`border rounded-2xl transition-all duration-300 ${openIndex === i ? 'border-[#1DE5B5] bg-[#F4FBFA]' : 'border-[#E6F0F7] bg-white hover:border-[#239bf5]/30'}`}>
                            <button
                                onClick={() => setOpenIndex(openIndex === i ? null : i)}
                                className="w-full text-left px-6 py-5 flex items-center justify-between gap-4"
                            >
                                <h4 className="font-bold text-[#09263F] text-sm leading-snug">{faq.question}</h4>
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${openIndex === i ? 'bg-[#1DE5B5] text-[#09263F]' : 'bg-[#F0F7FF] text-[#239bf5]'}`}>
