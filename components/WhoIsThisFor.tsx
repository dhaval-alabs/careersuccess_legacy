...
export default function WhoIsThisFor({ subtitle }: { subtitle?: string }) {
  ...
  return (
    <section id="who-is-this-for" className="py-16 bg-[#F4FAFA]">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <span className="inline-block bg-[#e8f4fd] text-[12px] font-bold uppercase tracking-[0.1em] px-[18px] py-[5px] rounded-full border border-[#b8ddf7] mb-4">
            <span className="bg-gradient-to-r from-[#19dfaf] to-[#07b2e8] bg-clip-text text-transparent">WHO IS THIS FOR</span>
          </span>
        ...
        <div ref={ref} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {PERSONAS.map((persona, i) => (
            <div
              key={persona.title}
              className="bg-white border border-[#D6ECEB] rounded-2xl p-6 flex flex-col transition-all duration-300 hover:shadow-lg"
              style={{
                opacity: visible ? 1 : 0,
